<!DOCTYPE html>
<html lang="en">

<head>
    <?php include "../includes/head.php"; ?>
</head>

<body>
    <?php include "../includes/header.php"; ?>
    <main>
        <div class="wrapper">
            <div class="page">
                <ul>
                    <li><a href="../">首頁</a></li>
                    <li><a href="javascript:void(0)">關於易兆</a></li>
                    <li><a href="javascript:void(0)">品牌哲學</a></li>
                </ul>
            </div>
            <div class="sidebar">
                <p><span>we know is international.</span><span>Only Standard</span></p>
                <h2>
                    <div class="zh">品牌哲學</div>
                    <div class="en">Brand philosophy<span class="slog">國際防水標準，我們唯一懂的標準。</span></div>
                </h2>
            </div>
            <div class="main_content">
                <div class="main_content_manu">
                    <a href="../material">材料與技術</a>
                    <a class="active" href="javascript:void(0)">專業團隊</a>
                </div>
                <div id="material" class="team">
                    <div class="item">
                        <h3>
                            <span>世界20大化工企業夥伴 Business partners</span>
                        </h3>
                        <div class="photo">
                            <div class="imgLiquidCC">
                                <img src="../image/about/shutterstock_209233546-1.jpg" alt="">
                            </div>
                        </div>
                        <div class="text">
                            <p>【 關於拜耳 】</p>
                            <p>與德國拜耳公司(Bayer)(現在更名為科思創Covestro) 、及德國明氏公司(MC Bauchemie)合作，引進最穩定成熟的施工技術及材料，引導業界正確的材料應用及精湛的施工品質，建立與德國相同水準的規範及標準。</p>
                            <p>在中國東北參與哈大高鐵混凝土防護工程，經過極端環境溫度凍融循環的反覆考驗。</p>
                            <p>在中國西南四川地區沿岷江高速公路橋墩防撞保護，經過桃花水的衝擊依然屹立不搖。</p>
                            <p>在新加坡海底20公尺的海底隧道防水，長期海水的浸泡依然堅守岡位。</p>
                            <p>在新加坡Eco-Link@BKE生態橋樑防樹根穿刺防水，提供安全廊道讓動物回家。</p>
                            <p>與大陸工程公司(CEC) 、潤弘精密(Ruentex)、富邦建設(Fubonland)建立緊密的合作夥伴關係。</p>
                        </div>
                    </div>
                    <div class="item video">
                        <h3>
                            <span>專業施工團隊 Professional team</span>
                        </h3>
                        <div class="text">
                            <p>【 拜鐵膜施工隊資格 】</p>
                            <ul>
                                <li>現在從事防水工程</li>
                                <li>有現場施工3年以上經驗</li>
                                <li>年齡40歲以下</li>
                                <li>有施工夥伴3人以上</li>
                                <li>有強烈意願提升防水施工品質</li>
                            </ul>
                        </div>
                        <div class="photo">
                            <div class="MYvideo">
                                <img src="../image/818.jpg" alt="video">
                                <video controls>
                                    <source src="../video/videoplayback.mp4" type="video/mp4">
                                </video>
                            </div>
                        </div>
                    </div>
                    <div class="item certificate">
                        <h4>【 施工人員證書認證 】</h4>
                        <div class="photo">
                            <div class="imgLiquidCC">
                                <img src="../image/about/shutterstock_209233546-1.jpg" alt="">
                            </div>
                        </div>
                        <div class="text">
                            <p>每一位施工人員皆需獲得「Applicator of 拜鐵膜® SPR」頒發證書，茲證明其參加拜鐵膜系統的施工訓練，並通過實際施作測驗及考核。</p>
                            <img src="../image/about/p3.png" alt="">
                            <p>
                                受訓課程主題包含：∙ 易壁固底漆及拜鐵膜系統; 材料及危害識別<br>
                                ∙ 兩把刷®雙組分噴塗設備; 使用和維護<br>
                                ∙ 混凝土、鋼底材及拜鐵膜; 表面處理<br>
                                ∙ 拜鐵膜系統; 噴塗、品質控制及檢測
                            </p>
                        </div>
                        <h5>國際防水標準，我們唯一懂的標準！</h5>
                    </div>
                    <div class="item partners">
                        <h3>
                            <span>海外夥伴 Internation partner</span>
                        </h3>

                        <div class="photo">
                            <img src="../image/about/p1.png" alt="">
                        </div>
                        <div class="text">
                            <p>
                                【 Lee construction Pte Ltd. 】<br>
                                Lee construction Pte Ltd.是一間位於新加坡的營建公司，創立於1995年，目前跨足工業、商業及住宅等領域，其中一些著名的項目是地鐵，水庫，學校，文萊蘇丹王府，工廠，商業和住宅，酒店，公寓和私人住宅以及陸路交通管理局。
                            </p>
                            <p>
                                除了在當地許多知名工程，該公司還承接了新加坡以外的項目，例如越南，柬埔寨，緬甸，文萊和中國。 並還獲得了ISO 9001：2008 和 OHSAS 18001：2007 的認證。
                            </p>
                        </div>
                        <div class="text content_text">
                            <p>與易兆相遇:<br />Lee construction於2008年承攬新加坡濱海高速公路 (Marina Coastal Expressway)MCE 485, 分兩次建造及防水施工，建築師在設計規劃防水是設計英國S公司的MMA材料，當Lee construction噴塗S公司的MMA材料在phase 1的隧道外面，完工拔樁放水之後漏水嚴重。</p>
                            <p>Lee construction 為了在Phase 2徹底解決防水問題，找上當時叫拜耳公司化工部門尋求協助解決海底隧道問題。<br />因為拜鐵膜最重要是施工技術問題，所以拜耳引薦了易兆與 Lee construction 認識，開啟了長期的合作之路。</p>
                        </div>
                        <h2 class="tc">【 合作的實績 】</h2>
                        <div class="slick">
                            <div class="photo_item">
                                <div class="imgLiquidCC">
                                    <img src="../image/about/shutterstock_209233546-1.jpg" alt="">
                                </div>
                                <p>(Marina Coastal Expressway)MCE 485</p>
                            </div>
                            <div class="photo_item">
                                <div class="imgLiquidCC">
                                    <img src="../image/about/shutterstock_209233546-1.jpg" alt="">
                                </div>
                                <p>ECO Link生態廊道橋樑防水</p>
                            </div>
                            <div class="photo_item">
                                <div class="imgLiquidCC">
                                    <img src="../image/about/shutterstock_209233546-1.jpg" alt="">
                                </div>
                                <p>新加坡香格里拉飯店景觀防水翻修</p>
                            </div>
                            <div class="photo_item">
                                <div class="imgLiquidCC">
                                    <img src="../image/about/shutterstock_209233546-1.jpg" alt="">
                                </div>
                                <p>ECO Link II bridge</p>
                            </div>
                        </div>

                        <h2>【 其他海外夥伴 】</h2>
                        <div class="vendor">
                            <div class="img"><img src="../image/about/Group9.png" alt=""></div>
                            <div class="img"><img src="../image/about/Group10.png" alt=""></div>
                            <div class="img"><img src="../image/about/Group11.png" alt=""></div>
                            <div class="img"><img src="../image/about/Group12.png" alt=""></div>
                        </div>
                        <h3><span>國內夥伴 Domestic partner </span></h3>
                        <div class="vendor">
                            <div class="img"><img src="../image/about/Group21.png" alt=""></div>
                            <div class="img"><img src="../image/about/Group13.png" alt=""></div>
                            <div class="img"><img src="../image/about/Group14.png" alt=""></div>
                            <div class="img"><img src="../image/about/Group15.png" alt=""></div>
                            <div class="img"><img src="../image/about/Group16.png" alt=""></div>
                            <div class="img"><img src="../image/about/Group17.png" alt=""></div>
                            <div class="img"><img src="../image/about/Group18.png" alt=""></div>
                            <div class="img"><img src="../image/about/Group19.png" alt=""></div>
                            <div class="img"><img src="../image/about/Group20.png" alt=""></div>
                        </div>
                    </div>
                    <div class="item"></div>
                </div>
            </div>
        </div>
    </main>
    <?php include "../includes/fotter.php"; ?>
    <?php include "../includes/fotter_js.php"; ?>
</body>

</html>